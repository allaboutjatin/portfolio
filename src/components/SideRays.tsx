...
void main() {
  ...
  gl_FragColor = color;
      ...
      const uniforms: any = {
        iTime: { value: 0 },
        iResolution: { value: [containerRef.current.clientWidth || window.innerWidth, containerRef.current.clientHeight || window.innerHeight] },
        iSpeed: { value: speed },
      ...
      const updateSize = () => {
        ...
        if (uniformsRef.current?.iResolution) {
          uniformsRef.current.iResolution.value = [w * dpr, h * dpr];
        }
      };
