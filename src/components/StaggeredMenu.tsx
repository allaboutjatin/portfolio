...
export const StaggeredMenu: React.FC<StaggeredMenuProps> = ({
  ...
  return (
    ...
    >
      ...
      <header
        className={`staggered-menu-header ${!headerVisible ? 'header-hidden' : ''} ${open ? 'menu-open' : ''}`}
              ...
              items.map((it, idx) => (
                <li className="sm-panel-itemWrap" key={idx}>
                  {it.onClick ? (
                    <button
                      type="button"
                      className="sm-panel-item text-left bg-transparent border-0 cursor-pointer p-0"
                      onClick={() => {
                        it.onClick?.();
                        closeMenu();
                      }}
                      aria-label={it.ariaLabel || (typeof it.label === 'string' ? it.label : undefined)}
                      data-index={idx + 1}
                    >
