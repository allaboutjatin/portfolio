...
export default function ThemeSwitchFlowGlass({
  ...
  return (
    <div className="flex flex-col items-center gap-3">
      <div
        className={cn(
          "relative flex h-11 w-28 select-none items-center justify-center cursor-pointer",
      ...
      >
        ...
        <div
          ...
          className={cn(
            ...
            "border-white/30 bg-black/20 shadow-inner",
          )}
          style={{ zIndex: 5 }}
        />

        {/* Real shadcn/ui Switch */}
        <Switch
          checked={checked}
          onCheckedChange={onChange}
          aria-label="Toggle visual theme"
          className={cn(
            "peer absolute inset-0 h-full w-full rounded-full cursor-pointer",
            "!bg-transparent data-[state=checked]:!bg-transparent data-[state=unchecked]:!bg-transparent",
            "!border-0 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-sky-400 focus-visible:ring-offset-2",
            // Thumb geometry
            "[&>span]:absolute [&>span]:top-1 [&>span]:left-1 [&>span]:h-9 [&>span]:w-9",
            "[&>span]:rounded-full [&>span]:bg-white/95 [&>span]:shadow-lg [&>span]:z-30",
            // Thumb micro-interactions
            "[&>span]:transition-transform [&>span]:duration-300",
            "hover:[&>span]:scale-[1.04] active:[&>span]:scale-[0.96]",
            // Travel
            "data-[state=unchecked]:[&>span]:translate-x-0",
            "data-[state=checked]:[&>span]:translate-x-[68px]"
          )}
          style={{ zIndex: 10 }}
        />

        {/* ICONS — positioned on thumb stops */}
        <span
          aria-hidden
          className="pointer-events-none absolute inset-y-0 left-[22px] -translate-x-1/2 z-20 flex items-center justify-center"
        >
          <SunIcon
            size={18}
            className={cn(
              "transition-all duration-300",
              checked ? "opacity-30 text-zinc-400 scale-90" : "opacity-100 text-amber-500 scale-110 rotate-12 drop-shadow-sm"
            )}
          />
        </span>
        <span
          aria-hidden
          className="pointer-events-none absolute inset-y-0 right-[22px] translate-x-1/2 z-20 flex items-center justify-center"
        >
          <MoonIcon
            size={18}
            className={cn(
              "transition-all duration-300",
              checked ? "opacity-100 text-sky-400 scale-110 -rotate-12 drop-shadow-sm" : "opacity-30 text-zinc-400 scale-90"
            )}
          />
        </span>
      </div>
